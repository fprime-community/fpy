...
If `timeout` is provided, and cannot be coerced to [`Fw.TimeIntervalValue`](todo), an error is raised.

If `persist` or `period` is provided, and they cannot be coerced to [`Fw.TimeIntervalValue`](todo), an error is raised.

At execution:
1. If provided, `timeout`, `persist` and `period` are evaluated and stored. `timeout`'s stored value is the current time plus the evaluated interval.
2. If `persist` is not provided, its stored value is a zero-duration `Fw.TimeIntervalValue`.
3. If `period` is not provided, its stored value is a one-second `Fw.TimeIntervalValue`.
...
7. Otherwise, [sleep](todo) for `period`'s stored duration.
8. Go to step 4.
...
#### Semantics
At evaluation:
1. If `exit_code` evaluates to a non-zero value, the sequence ends with an error.
2. Otherwise, the sequence ends without error.
