...
//!
//! On success it prints `<channel> <code>` on the final line of stdout and
//! exits 0, where the i32 error code is a decimal and the channel names where
//! it came from:
//!
//!   * `exit <code>`  -- `fprime_v1.exit`: the sequence called exit() or
//!     failed an assert, or `main` returned normally (code 0).
//!   * `panic <code>` -- `fprime_v1.panic`: an implicit runtime check fired
//!     (array index out of bounds, zero divisor, arithmetic overflow),
//!     carrying a DirectiveErrorCode. Kept separate from `exit` so a sequence
//!     calling exit(10) can never impersonate a DOMAIN_ERROR panic.
//!
//! The entry itself is a `void main()` -- no arguments, no return value --
//! since every failure (and explicit exit) leaves through exit/panic and
//! never comes back, so returning at all means success (reported as
//! `exit 0`). The caller reads the printed line; the process exit status only
...
/// into their own channel's cell (*exit_code* / *panic_code*) and unwind the
/// interpreter with a host trap, so they terminate the program from any call
/// depth (a `ret` would only unwind the current function). The recorded code
/// -- not the trap -- carries the result. `exit` is a termination the sequence
/// asked for (exit(), a failing assert; code 0 is a normal exit, nonzero an
/// error), `panic` an implicit runtime trap (array index out of bounds, zero
/// divisor, arithmetic overflow; always an error). The two cells are kept
/// separate so a sequence calling exit(10) can never impersonate a
/// DOMAIN_ERROR panic, whose code is also 10.
///
...
#[derive(Clone, Copy)]
enum Outcome {
    /// The `exit` channel: exit(), a failing assert, or main returning
    /// normally (code 0).
    Exit(i32),
    /// The `panic` channel: an implicit runtime check raised a
    /// DirectiveErrorCode (zero divisor, arithmetic overflow, ...).
    Panic(i32),
}
...
fn run(
    ...
    // exit/panic write the sequence's error code into their own cell and
    // unwind the interpreter.
    let exit_code: Rc<Cell<Option<i32>>> = Rc::new(Cell::new(None));
    let panic_code: Rc<Cell<Option<i32>>> = Rc::new(Cell::new(None));
    let outcome = {
        let exit_code = exit_code.clone();
        let panic_code = panic_code.clone();
        move || {
            // A panic is never followed by more guest code (it unwinds), so at
            // most one of the two cells is ever set.
            panic_code
                .get()
                .map(Outcome::Panic)
                .or_else(|| exit_code.get().map(Outcome::Exit))
        }
    };
    ...
    match result {
        ...
        InterpreterResult::Finished => Ok(Outcome::Exit(0)),
