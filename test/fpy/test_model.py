...
class TestIntegerOverflow:
    ...
    def test_smod_int_min_by_minus_one_overflows(self):
        """MIN_INT64 % -1 traps like MIN_INT64 // -1. The mathematical
        remainder 0 is representable, but the operation is undefined behavior
        in C++/LLVM, and Rust's `%` (like C#'s) overflows on it -- only the
        explicit wrapping_rem opts out. `//` and `%` halt on exactly the same
        inputs; see MATH_COMPARISON.md."""
        model = self._make_model()
        model.push(MIN_INT64)
        model.push(-1)
        result = model.dispatch(SignedModuloDirective())
