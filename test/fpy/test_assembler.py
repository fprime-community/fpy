...
def get_all_concrete_directive_classes() -> set[type]:
    """Get all concrete (non-abstract) directive classes that can be serialized."""
    concrete_classes = set()
    
    # Get direct subclasses of Directive (excluding StackOpDirective which is abstract)
    for cls in Directive.__subclasses__():
        if cls is StackOpDirective:
            # StackOpDirective is abstract, get its concrete subclasses
            for stack_cls in StackOpDirective.__subclasses__():
                concrete_classes.add(stack_cls)
        else:
            concrete_classes.add(cls)
    
    return concrete_classes


# Set of all directive classes that are tested in TestDirectiveSerializationRoundTrip
TESTED_DIRECTIVE_CLASSES: set[type] = {
    # No-arg directives
    NoOpDirective,
    WaitRelDirective,
    WaitAbsDirective,
...
class TestDirectiveCoverage:
    """Ensure all directive classes are covered by the serialization tests."""

    def test_all_directives_are_tested(self):
        """Verify that every concrete directive class has a serialization test."""
        all_directives = get_all_concrete_directive_classes()
        untested = all_directives - TESTED_DIRECTIVE_CLASSES
        
        assert not untested, (
            f"The following directive classes are not tested in "
            f"TestDirectiveSerializationRoundTrip:\n"
            + "\n".join(f"  - {cls.__name__}" for cls in sorted(untested, key=lambda c: c.__name__))
            + "\n\nPlease add tests for these directives and add them to TESTED_DIRECTIVE_CLASSES."
        )

    def test_no_extra_classes_in_tested_set(self):
        """Verify that TESTED_DIRECTIVE_CLASSES doesn't contain non-existent classes."""
        all_directives = get_all_concrete_directive_classes()
        extra = TESTED_DIRECTIVE_CLASSES - all_directives
        
        assert not extra, (
            f"The following classes in TESTED_DIRECTIVE_CLASSES are not valid directive classes:\n"
            + "\n".join(f"  - {cls.__name__}" for cls in sorted(extra, key=lambda c: c.__name__))
        )
