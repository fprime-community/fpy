...
@dataclass
class SequenceSymbol(CallableSymbol):
    """A callable symbol representing an imported sequence.
    
    The sequence is invoked at runtime via a command directive.
    Its return type is always Fw.CmdResponse.
    """
    file_path: str
    """Resolved absolute path to the .fpy source file"""
    dotted_path: str
    """The dotted import path (e.g. 'path.to.seq')"""
...
class CompileState:
    ...
    sequence_arg_size: int = 0
    """total size in bytes of sequence arguments (pushed by VM, not ALLOCATEd)"""
