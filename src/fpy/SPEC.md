...
* `write_to_port(port: FwIndexType, value)`: pushes `value` onto the stack and emits a `PopSerializableDirective` that pops those bytes and sends them out the sequencer's `serialOut[port]` port. `value` may be any *serializable, statically-sized* expression: it must have a binary representation and a compile-time-known serialized size (so runtime-variable-length strings are rejected, but a constant string literal whose length is known at compile time — is allowed).
