...
Accessing `Fw.TimeValue` produces synthetic members named `timeBase`, `timeContext`, `seconds`, and `useconds` with types `TimeBase` (enum), `FwTimeContextStoreType` (U8), `U32`, and `U32` respectively, matching the dictionary's `Fw.TimeValue` definition. `Fw.Time` is an alias for `Fw.TimeValue`.
...
* `iabs(value: I64) -> I64`: returns the absolute value of a signed 64-bit integer by emitting an `IntAbsDirective`. The absolute value of `I64` min (`-2**63`) is not representable in `I64`, so it raises an `ARITHMETIC_OVERFLOW` runtime error.
* `fabs(value: F64) -> F64`: returns the absolute value of a 64-bit float by emitting a `FloatAbsDirective`. Clears the sign bit and changes nothing else (the IEEE 754 `abs` operation): `-0.0` becomes `0.0`, `-inf` becomes `inf`, and a NaN keeps its payload and signaling bit.
...
* `write_to_port(port: Svc.Fpy.SerialPortIndex, value)`: pushes `value` onto the stack and emits a `PopSerializableDirective` that pops those bytes and sends them out the sequencer's `serialOut[port]` port. `port` must be a compile-time-constant `Svc.Fpy.SerialPortIndex` enum constant, which resolves to its integer index in the emitted directive. `Svc.Fpy.SerialPortIndex` is defined in the loaded dictionary (from `FpySequencerCfg`) and is project-customizable — its constant names are whatever the deployment defines (the reference names `EXAMPLE_PORT_0..4` are meant to be renamed per application), not a compiler-hardcoded set. `value` may be any *serializable, statically-sized* expression: it must have a binary representation and a compile-time-known serialized size (so runtime-variable-length strings are rejected, but a constant string literal whose length is known at compile time — is allowed).
