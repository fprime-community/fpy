...
* `iabs(value: I64) -> I64`: returns the absolute value of a signed 64-bit integer by emitting an `IntAbsDirective`. The absolute value of `I64` min (`-2**63`) wraps back to `I64` min rather than trapping (two's-complement negation, matching `llvm.abs` with `is_int_min_poison=0`).
